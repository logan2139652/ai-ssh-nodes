<p align="center">
  ...
  <p align="center"><strong>AI 原生基础设施控制平面</strong></p>
  <p align="center">一个 YAML 文件管理所有服务器 · AI 原生理解拓扑 · 零 Agent / 零 Daemon</p>
  <p align="center">内嵌 <strong>WorkBuddy / Codex / Generic-Agent</strong> 三平台 Skill 适配</p>
</p>
...
内置 **WorkBuddy / Codex / 通用 Agent** 三套 Skill 适配。
...
├── skills/
│   ├── README.md                     # 多平台适配说明
│   ├── workbuddy/SKILL.md            # WorkBuddy 索引 → 根 SKILL.md
│   ├── codex/
│   │   ├── INSTALL.md                # Codex 安装指南（英文）
│   │   ├── INSTALL.zh.md             # Codex 安装指南（中文）
│   │   └── ai-ssh-nodes/
│   │       ├── SKILL.md              # Codex 原生 Skill
│   │       └── agents/openai.yaml    # Agent UI 元数据
│   └── generic-agent/
│       └── SKILL.md                  # 通用 prompt 版本
└── workspace/
...
使用 `# >>> ai-ssh-nodes managed` / `# <<< ai-ssh-nodes managed` 标记区块，区块内由脚本自动管理，区块外的手动配置不受影响。
...
生成的 workspace 包含：本地共享工作区、每台服务器的远程目录、AI Context 注入、预置 VSCode Tasks。
...
`skills/` 目录包含针对不同平台的 Skill 适配：

| 平台 | 路径 | 使用方式 |
|----------|------|------------|
| **WorkBuddy** | `SKILL.md`（根目录） | 通过 WorkBuddy 侧边栏导入 |
| **Codex** | `skills/codex/ai-ssh-nodes/` | 复制到 `~/.codex/skills/` — 见安装指南 |
| **通用 Agent** | `skills/generic-agent/SKILL.md` | 作为 system prompt 或 rules 文件注入 |

所有适配共享同一份 `workspace/servers.yaml` 和 `scripts/`，区别仅在于 AI 如何被"教会"使用它们。

### 适配 Cursor / Claude Code / Copilot / Cline

将以下核心指令块写入项目的 rules 文件中：
...
- 禁止命令：rm -rf /、dd if=、mkfs、shutdown/reboot、fork bomb、curl|bash（未审查）
